module.exports = function(RED) {
  ...
  function UpdateOwnerFromSdk(thisNode, msg, return_promise) {
    ...
    if (typeof msg.payload[0] == 'string') {
      //[username, body] Update a single Object
  ...
  function ClaimOrUnClaimOwnerFromSdk(thisNode, msg, action, return_promise) {
    ...
    if (msg.payload.every((el) => typeof el == 'string')) {
      //[username, deviceid] Single Claim/ UnClaim
      if (msg.payload.length == 2) {
        var owner = msg.payload[0];
  ...
  RED.httpAdmin.post('/owners/:id/button', RED.auth.needsPermission('mnubo owners.write'), function mnubo_owners_button(
    req,
    res
  ) {
    ConfigMnuboUtils.DebugLog();
