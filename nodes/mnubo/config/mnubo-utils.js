function UpdateStatus(thisNode) {
  ...
  if (thisNode.mnuboconfig.credentials_type === 'app_token') {
    ...
    if (
      myCredential.id == null ||
      myCredential.id === '' ||
      myCredential.secret == null ||
      myCredential.secret === ''
    ) {
...
function CheckMultiStatusResult(thisNode, data, request) {
  ...
  if (failed_events.length > 0) {
    // Some errors
    if (failed_events.length === request.length) {
...
function ProxyUrl2HttpOptions(mnuboconfig) {
  ...
  var port = url.parse(mnuboconfig.proxy_url).port
    ? url.parse(mnuboconfig.proxy_url).port
    : protocol === 'https'
    ? 443
    : 80;
  return {
