module.exports = function(RED) {
      ...
      client.model
        ...
        .catch(function GetModelFromSdk_ERR(error) {
          ...
          msg.errors = {
            ...
            originalRequest: { acction: 'Get Data Model', mnuboconfig: thisNode.mnuboconfig },
          };
          thisNode.send(msg);
        });
    }
        ...
        if (objectType.hasOwnProperty('objectAttributes')) {
          objectType.objectAttributes.map((object_attribute) => {
            if (!object_attributes.find((attribute) => attribute.key === object_attribute.key)) {
              ...
              object_attributes
                .find((attribute) => attribute.key === object_attribute.key)
                .objectTypeKeys.push(objectType.key);
            }
          });
        }
      });
    ...
    if (data_model.event_types.length > 0 || data_model.object_types.length > 0) {
      client.model
        ...
        .then((currentDataModel) => {
          ...
          new_event_types = data_model.event_types.filter(
            (item) => !current_data_model.event_types.some((other) => item.key === other.key)
          );
          new_object_types = data_model.object_types.filter(
            (item) => !current_data_model.object_types.some((other) => item.key === other.key)
          );
          new_timeseries = data_model.timeseries.filter(
            (item) => !current_data_model.timeseries.some((other) => item.key === other.key)
          );
          new_object_attributes = data_model.object_attributes.filter(
            (item) => !current_data_model.object_attributes.some((other) => item.key === other.key)
          );
          new_owner_attributes = data_model.owner_attributes.filter(
            (item) => !current_data_model.owner_attributes.some((other) => item.key === other.key)
          );
    ...
    return Promise.all(deployementPromises).then((done) => {
      ...
      if (failed.length > 0) {
        ...
        if (done.length === failed.length) {
          ...
          msg.errors = {
            errorMessage: failed,
            originalRequest: { acction: 'Deploy to Production', payload: msg.payload },
          };
          msg.payload = 'failed';
          thisNode.send(msg);
        } else {
  ...
  function ResetDataModel(thisNode, msg, return_promise) {
    ...
    if (return_promise == 1) {
      ...
      client.model.sandboxOps.resetOps
        .reset()
